...
public class BlackJack {
    ...
    public static void main(String[] args) {
        ...
        //The main gameflow in the try block, it catch the exception and end the game peacefully without winner or loser when the deck is empty
        try {
            /*
                Stage1: Game Start
            ...
            for (int i = 0; i < numOfPlayer; i++) {
                System.out.println("Player " + (i + 1) + "'s Hand: " + players[i].getPlayerInfo());
            ...
            if (startDealerRound) {
                ...
                //Show dealer's hands
                System.out.println("Dealer's Hand: " + dealer.getPlayerInfo());
            ...
            //Terminate the game without winner when deck is empty
            System.out.println("\nFinal Result (" + numOfPlayer + " players)");
            System.out.println(LINE);

            //Print player and dealer's final hands
            for (int i = 0; i < numOfPlayer; i++) {
                System.out.println("Player " + (i + 1) + "'s Hand: " + players[i].getPlayerInfo());
            }
            System.out.println("Dealer's Hand: " + dealer.getPlayerInfo());
    ...
    public static int inputAndValidate(Function<Integer, Boolean> condition, String prompt, String msgErr, Scanner scanner) {
        ...
        do {
            ...
            try { //to catch exception of the non-int input
                input = scanner.nextInt(); //wait for input
                if (!condition.apply(input)) { //Determine input is valid
                    System.out.println(msgErr); //Error message if input is not valid
                }
            } catch (Exception e) {
     ...
     */
    public static int[] arrayAddItem(int[] originArray, int itemToAdd) {
